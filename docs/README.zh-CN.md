...
- **Agent Skill 安装器**：执行 **Edo Tensei: 安装 Agent Skill** 安装正式 `edo-tensei` skill。只有需要自行写入指定 agent 的 skill/rule 文件时，才使用 **Generate Skill Files Manually**。
...
| 安装 Agent Skill | 安装正式 `edo-tensei` skill；只有需要指定 agent 文件时，才在命令内使用 **Generate Skill Files Manually** |
...
执行 **Edo Tensei: 安装 Agent Skill**，选择 **Auto Install (Recommended)** 安装正式 `edo-tensei` skill。也可以直接执行 `npx skills add Pain-Labs/Edo-Tensei`。只有需要自行写入指定 agent 的 skill/rule 文件时，才选择 **Generate Skill Files Manually**。生成的内容不是松散备忘，而是一份结构化 SOP：告诉接手代理如何定位可能的 session 文件、只读取最近且相关的片段、在置信度不足时停止，并输出干净的交接摘要。
...
- **Antigravity**：从 `transcript.jsonl` 或 `overview.txt`（预览 log）提取，每条消息截断于约 900 字符。完整对话历史仅存于 Antigravity 云端，本地无法访问。
