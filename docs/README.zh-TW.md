...
- **Agent Skill 產生器**：為 Claude Code、GitHub Copilot、Kiro、Antigravity、Cline、Gemini CLI 與 Cursor 產生可重用的 `edo-tensei` skill/rule 檔案。
...
使用 **Generate Agent Skill** 可為其他 AI 工具建立可重用的 `edo-tensei` skill 或 rule。產生出的內容不是零散備忘錄，而是一份結構化 SOP：告訴接手代理如何定位可能的 session 檔案、只讀最近且相關的片段、在信心不足時停止，並輸出乾淨的交接摘要。
